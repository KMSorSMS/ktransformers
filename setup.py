...
class VersionInfo:
    ...
    BASE_WHEEL_URL: str = "https://github.com/kvcache-ai/ktransformers/releases/download/{tag_name}/{wheel_filename}"
    ...
    def get_cpu_instruct(
        ...
        if sys.platform.startswith("linux"):
            ...
            if CPUFeature.get("AVX2", False):
                return "avx2"
            raise ValueError("Unsupported cpu Instructions: {}".format(str(CPUFeature)))
        else:
            raise ValueError("Unsupported platform: {}".format(sys.platform))
...
class CMakeBuild(BuildExtension):
    ...
    def build_extension(self, ext) -> None:
        ...
        cmake_args += [item for item in cpu_args.split(" ") if item]

        ...
        async def inner():
            async for token in interface.inference(input.prompt, id):
                d = OllamaGenerationStreamResponse(
                    model=config.model_name, created_at=str(datetime.now()), response=token, done=False
                )
            ...
            # yield f"{json.dumps(d)}\n"
            d = OllamaGenerationStreamResponse(
                model=config.model_name, created_at=str(datetime.now()), response="", done=True
            )
            yield d.model_dump_json() + "\n"
...
async def show(request: Request, input: OllamaShowRequest):
    ...
    return OllamaShowResponse(
        ...
        template=" ",
        details=OllamaShowDetial(
            parent_model=" ", format="gguf", family=" ", families=[" "], parameter_size=" ", quantization_level=" "
        ),
        model_info=OllamaModelInfo(),
    )

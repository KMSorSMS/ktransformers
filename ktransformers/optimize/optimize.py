            ...
            if child_prefix in local_optimization_dict:
                ...
                if inject_module_meta["class"] != "default":
                    ...
                    gguf_loader.tensor_device_map[inject_module_meta["key"]] = (
                        inject_module_meta["kwargs"] if "kwargs" in inject_module_meta else dict()
                    )
                ...
                child_optimization_dict = {
                    k: v for k, v in local_optimization_dict.items() if k.startswith(child_prefix)
                }
...
def gen_optimize_config(
    module: nn.Module, out_data: Mapping, rule_list: List, prefix: str = "", default_device: str = "cuda:0"
):
        ...
        if "replace" in rule:
            ...
            if module_name not in out_data:
                ...
                out_data[module_name]["kwargs"].update(
                    copy.deepcopy(replace_meta["kwargs"]) if "kwargs" in replace_meta else dict()
                )
    ...
    if module_name not in out_data:
        out_data[module_name] = {
            "class": "default",
            "key": translated_name,
            "kwargs": {"generate_device": default_device, "prefill_device": default_device},
        }
...
def optimize_and_load_gguf(
    module: nn.Module, rule_file: str, gguf_path: str, model_config: PretrainedConfig, default_device: str = "cuda:0"
):
